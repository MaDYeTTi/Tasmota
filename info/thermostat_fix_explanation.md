...
- The `InputSwitchUse` command now validates the thermostat index just like other thermostat setters and only accepts `0` or `1`, so you can reliably enable or disable input-triggered manual mode on any controller.
- A new `Thermostat<x>` command (for example, `Thermostat1`) reports each controller's configured input switch, relay, output enable flag, and their current states, refreshed from the live switch/relay readings. This makes it easier to confirm that multiple thermostats are mapped to distinct hardware and to spot unintended overlaps.
